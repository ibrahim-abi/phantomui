 ...
 * Assembles the IIFE browser bundle from source modules -> dist/ai-sdk.js
 */
...
// Read source modules (strip require/module.exports boilerplate for inlining)
const attributesSrc = fs.readFileSync(path.join(rootDir, 'src/attributes.js'), 'utf-8');
const scannerSrc    = fs.readFileSync(path.join(rootDir, 'src/scanner.js'),    'utf-8');
const autotaggerSrc = fs.readFileSync(path.join(rootDir, 'src/autotagger.js'), 'utf-8');
const serializerSrc = fs.readFileSync(path.join(rootDir, 'src/serializer.js'), 'utf-8');

// Extract ATTRS value from attributes.js
const attrsMatch = attributesSrc.match(/module\.exports\s*=\s*(\{[\s\S]*?\});/);
const attrsBody  = attrsMatch ? attrsMatch[1] : '{}';
...
fs.writeFileSync(path.join(distDir, 'ai-sdk.js'), bundle, 'utf-8');

const sizeKb = (Buffer.byteLength(bundle, 'utf-8') / 1024).toFixed(1);
console.log('Built sdk/dist/ai-sdk.js — ' + sizeKb + ' KB');
